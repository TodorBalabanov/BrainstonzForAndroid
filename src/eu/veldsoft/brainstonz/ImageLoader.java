...
class ImageLoader {
	...
	private static final String[] glyphNames = { "a", "b", "c", "d", "e", "f",
			"g", "b", "g", "h", "f", "e", "c", "d", "a", "h" };
	private static final String glyphExt = ".png";
	public static BufferedImage[] glyphs = new BufferedImage[16];

	private static final String blackStoneName = "bs", whiteStoneName = "ws",
			stoneExt = ".png";
	...
	/**
	 * Loads the images for the Brainstonz GUI. Should be called before
	 * instantiating any GUI components.
	 * 
	 * @throws IOException
	 *             if loading fails
	 */
	public static void load() throws IOException {
		...
		// Load Background
		background = ImageIO.read(ImageLoader.class
				.getResourceAsStream(backgroundName));
		...
		for (int i = 0; i < 16; i++) {
			if (glyphs[BrainstonzState.pair[i]] != null) {
				...
				glyphs[i] = ImageIO.read(ImageLoader.class
						.getResourceAsStream("/" + glyphNames[i] + glyphExt));
			}
		}

		// Load Stonz
		for (int i = 0; i < numStoneImgs; i++) {
			blackStonz[i] = ImageIO.read(ImageLoader.class
					.getResourceAsStream("/" + blackStoneName + i + stoneExt));
			whiteStonz[i] = ImageIO.read(ImageLoader.class
					.getResourceAsStream("/" + whiteStoneName + i + stoneExt));
			for (int j = 1; j < 4; j++) {
		...
		// Load Question Icon
		questionIcon = new ImageIcon(ImageIO.read(ImageLoader.class
				.getResourceAsStream(questionIconName)));

	}

	/**
	 * Rotates a {@code BufferedImage} by a specified angle. Used to add variety
	 * to the stonz.
	 * 
	 * @param img
	 *            image to rotate
	...
	public static BufferedImage rotate(BufferedImage img, int angle) {
		...
		BufferedImage dimg = new BufferedImage(w, h,
				img.getType() == 0 ? BufferedImage.TYPE_INT_ARGB
						: img.getType());
