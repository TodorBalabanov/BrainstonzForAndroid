...
class BrainstonzState {
	...
	// Powers of Three
	public static final int[] p3 = { 1, 3, 9, 27, 81, 243, 729, 2187, 6561,
			19683, 59049, 177147, 531441, 1594323, 4782969, 14348907 };

	public static int get(int state, int position) {
		return (state / p3[position]) % 3;
	}
	...
	public static final int[] pair = { 14, 7, 12, 13, 11, 10, 8, 1, 6, 15, 5,
			4, 2, 3, 0, 9 };

	public static int getPair(int state, int position) {
		return get(state, pair[position]);
	}
		...
		// Otherwise, up to four sub-moves
			...
			for (int i = 0; i < 16; i++) {
				if (get(state, i) == 0) {
					...
					if (terminal(temp.state) != 0) {
						succs.add(temp);
					} else {
						// If the move makes a pair, remove a piece (succs1)
						if (get(state, pair[i]) == player) {
							succs1.add(temp);
						}
			...
			while (succs2.size() > 0) {
				...
				for (int i = 0; i < 16; i++) {
					if (get(temp2.state, i) == 0) {
						...
						temp.moves[2] = i;
						// If terminal, it is a win, so add it immediately to
						// successors
						if (terminal(temp.state) != 0) {
							succs.add(temp);
						} else {
							// If the move makes a pair, remove an opponent's
							// piece (succs3)
							if (get(temp2.state, pair[i]) == player) {
								succs3.add(temp);
							}
							// Otherwise, this turn is over, so add it to
							// successors list
							else {
								succs.add(temp);
							}
				...
				// If no spot to move, this is a tie, so add it to successors as
				// is
				if (!foundOne) {
					succs.add(temp2);
				}
